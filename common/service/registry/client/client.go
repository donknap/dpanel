...
import (
	...
	"github.com/donknap/dpanel/common/service/registry/client/interceptor"
	http2 "github.com/donknap/dpanel/common/service/registry/http"
	"github.com/donknap/dpanel/common/service/registry/types"
	"github.com/donknap/dpanel/common/service/registry/util"
...
func init() {
	...
	if env := os.Getenv("REGISTRY_HTTP_CLIENT_TIMEOUT"); len(env) > 0 {
		...
		if err != nil {
			slog.Error(fmt.Sprintf("Failed to parse REGISTRY_HTTP_CLIENT_TIMEOUT: %v, use default value: %v", err, DefaultHTTPClientTimeout))
		} else {
			if timeout > 0 {
...
func NewClientWithAuthorizer(url string, authorizer types.Authorizer, insecure bool, interceptors ...interceptor.Interceptor) Client {
	return &client{
		url:          url,
		authorizer:   authorizer,
		interceptors: interceptors,
		client: &http.Client{
			Transport: http2.GetHTTPTransport(http2.WithInsecure(insecure)),
...
func (c *client) DeleteManifest(repository, reference string) error {
	...
	if err != nil {
		...
		if !exist {
			return errors.New(fmt.Sprintf("%s:%s not found", repository, reference))
		}
...
func (c *client) Copy(srcRepo, srcRef, dstRepo, dstRef string, override bool) error {
	...
	if exist {
		...
		if !override {
			return errors.New("the same name but different digest artifact exists, but the override is set to false")
		}
	}
...
func (c *client) do(req *http.Request) (*http.Response, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		...
		code := types.GeneralCode
		switch resp.StatusCode {
		case http.StatusUnauthorized:
			code = types.UnAuthorizedCode
